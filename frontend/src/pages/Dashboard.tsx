  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="p-8 max-w-7xl mx-auto space-y-8 animate-fade-in w-full">
        ...
        {/* ── CSV Import Widget ───────────────────────────────────────────── */}
        <section className="bg-white border border-outline-variant/20 rounded-2xl shadow-sm overflow-hidden">
          {/* Header bar */}
          <div
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-sm text-on-surface">Import Meta Ads CSV</h4>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="material-symbols-outlined text-stone-400 text-[20px] transition-transform duration-200" style={{ transform: csvExpanded ? "rotate(180deg)" : "rotate(0deg)" }}>
                expand_more
              </span>
            </div>
          </div>
          ...
          {/* Collapsible body */}
          {csvExpanded && (
            <div className="border-t border-outline-variant/10 animate-slide-up">
              ...
              {!csvResult && (
                <div
                  className={`m-5 border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center transition-all ${
                    csvFile ? "border-primary bg-primary/5" : "border-outline-variant hover:border-primary/50 hover:bg-stone-50"
                  }`}
                  onDragOver={(e) => e.preventDefault()}
                  onDrop={onDrop}
                >
                  <span className="material-symbols-outlined text-5xl text-primary/30 mb-3">cloud_upload</span>
                  ...
                  <div className="flex gap-3">
                    <button
                      className="px-4 py-2 border border-outline-variant/40 rounded-lg text-xs font-bold text-stone-600 hover:bg-stone-100 transition-colors"
                    ...
                    {csvFile && (
                      <button
                        className="px-4 py-2 bg-primary text-white rounded-lg text-xs font-bold hover:opacity-90 transition-opacity disabled:opacity-50"
                  ...
                  {csvUploading && (
                    <div className="w-full max-w-xs bg-stone-100 rounded-full h-1.5 mt-4 overflow-hidden">
                      <div className="bg-primary h-1.5 rounded-full transition-all duration-300" style={{ width: `${csvProgress}%` }} />
                    </div>
                  )}
                  {csvError && (
                    <p className="text-xs text-error mt-3 bg-error/10 px-3 py-1.5 rounded-lg">{csvError}</p>
                  )}
                </div>
              )}
                      ...
                      <div className="h-44">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            ...
                            <RTooltip formatter={(v: number) => [`₹${v.toLocaleString()}`, "Spend"]} />
                          </PieChart>
                        </ResponsiveContainer>
                      </div>
