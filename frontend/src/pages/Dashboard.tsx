...
import { InteractiveSage } from "../components/dashboard/InteractiveSage";
        ...
        <section className="flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-1">
              <h2 className="font-serif text-4xl md:text-5xl font-bold tracking-tight text-on-surface">Overview</h2>
          ...
          <div className="flex gap-3">
            <button
              onClick={runCycle}
              disabled={cycling}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-xl text-sm font-bold hover:opacity-90 transition-all shadow-sm disabled:opacity-50"
            >
            ...
            <button className="flex items-center gap-2 px-4 py-2 bg-surface-container border border-outline-variant/30 text-stone-400 rounded-xl text-sm font-medium hover:bg-surface-container-high hover:text-white transition-colors shadow-sm">
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {priorityCampaigns.map((campaign) => (
              <div key={campaign.id} className="group bg-surface-container rounded-xl border border-outline-variant/20 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
                <div className="h-40 bg-gradient-to-br from-primary/20 to-surface-container-high flex items-center justify-center relative">
                  <span className="material-symbols-outlined text-6xl text-white/10">campaign</span>
                  <div className="absolute top-4 right-4">
                    <span className="px-2 py-1 bg-surface-container-highest/90 backdrop-blur text-primary text-[10px] font-bold uppercase rounded-md shadow-sm border border-outline-variant/10">
                      {campaign.status}
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  ...
                  <button className="w-full py-2 bg-surface-container-high text-stone-400 text-[10px] font-bold uppercase tracking-widest rounded-lg group-hover:bg-primary group-hover:text-white transition-colors">
                    Manage Observatory
                  </button>
                </div>
              </div>
